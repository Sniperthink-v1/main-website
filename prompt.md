Combine the three provided React apps—`main-website`, `6LayersLP`, and `aiagents`—into a single deployable React project. The resulting architecture should serve each sub-project at the following routes:

- `main-website` at: https://localhost:3000/
- `6LayersLP` at: https://localhost:3000/6LayersLP
...
2. The 'start scaling' button in the `products` section of `main-website` must link to `/6LayersLP` and `/aiagents` using proper React Router navigation.
...
You will receive folders for `main-website`, `6LayersLP`, and `aiagents` as seen in the project folder.