...
export default function ProductShowcaseSection() {
  ...
  const nextCard = () => {
    setActiveCard((prev) => (prev + 4) % products.length);
  };

  const prevCard = () => {
    setActiveCard((prev) => (prev - 4 + products.length) % products.length);
  };
  ...
  // Handle mouse movement for sinusoidal effect
  useEffect(() => {
  ...
  // Calculate sinusoidal transform for each card
  const calculateSinusoidalTransform = (cardIndex: number, totalCards: number) => {
    if (!isHovering) return { x: 0, y: 0, rotation: 0 };
    
    // Normalize card position (0 to 1)
    const normalizedPosition = cardIndex / (totalCards - 1);
    
    // Calculate phase based on mouse position (0 to 2π)
    const phase = mouseX * Math.PI * 2;
    
    // Calculate sine wave with the card's position
    const sineValue = Math.sin(normalizedPosition * Math.PI * 2 - phase);
    
    // Map sine value to vertical displacement (center card goes down)
    const yDisplacement = sineValue * 50; // Amplitude of 50px
    
    // Horizontal displacement (moves opposite to mouse)
    const xDisplacement = (mouseX - 0.5) * -120; // Max 60px left or right
  ...
  return (
    ...
    >
      ...
      >
        <div className="grid grid-cols-4 gap-8">
          {getVisibleCards().map((product, index) => {
            ...
            const transform = calculateSinusoidalTransform(index, 4);
            
            return (
              <div key={product.id} className="w-full">
                <div
                  className={`flex flex-col w-full items-start justify-center gap-6 p-8 relative rounded-2xl transition-all duration-300 ease-out min-h-[320px] ${
                    isHovered
                      ? 'bg-gradient-to-b from-[rgba(252,230,178,0.4)] to-[rgba(210,159,52,0.4)] scale-110 shadow-2xl border-2 border-[#E1A940]/50'
                      : 'bg-white/5 backdrop-blur-sm border border-white/10 hover:bg-white/10 hover:shadow-lg hover:border-white/20'
                  }`}
                  onMouseEnter={() => setHoveredCard(originalIndex)}
                  onMouseLeave={() => setHoveredCard(null)}
                  style={{
                    transform: `translate(${transform.x}px, ${transform.y}px) ${isHovered ? 'scale(1.1)' : 'scale(1)'}`,
                    transition: 'transform 0.3s ease-out, background 0.3s ease-in-out, scale 0.3s ease-in-out, border 0.3s ease-in-out',
                    zIndex: isHovered ? 10 : 1
                  }}
                >
                  {/* Icon */}
                  <div className={`w-16 h-16 bg-gradient-to-r from-[#1A6262] to-[#91C499] rounded-xl flex items-center justify-center mb-4 shadow-sm transition-all duration-300 ${
                  ...
                  <div className="flex flex-col gap-4 w-full">
                    <h3 className={`font-semibold text-white text-xl leading-7 transition-all duration-300 ${
                      isHovered ? 'text-[#E1A940]' : ''
                    }`}>
                      {product.title}
                    </h3>
                    <p className={`text-white/70 text-base leading-6 transition-colors duration-300 ${
      ...
      <div 
        className="hidden md:block lg:hidden relative w-full"
        onMouseEnter={() => setIsHovering(true)}
        onMouseLeave={() => setIsHovering(false)}
      >
        <div className="grid grid-cols-2 gap-8">
          {getVisibleCards().slice(0, 2).map((product, index) => {
            ...
            const isActive = originalIndex === activeCard;
            const isHovered = hoveredCard === originalIndex;
            const transform = calculateSinusoidalTransform(index, 2);
            ...
            return (
              <div key={product.id} className="w-full">
                ...
                >
                  {/* Icon */}
                  <div className={`w-16 h-16 rounded-xl bg-gradient-to-r ${product.color} flex items-center justify-center text-2xl transition-all duration-300 ${
                    isHovered ? 'shadow-lg scale-110' : ''
                  }`}>
                    {React.createElement(product.icon, { className: "w-8 h-8 text-black" })}
                  </div>
