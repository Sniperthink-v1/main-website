  ...
  return (
    <div
      ...
      className={`flex flex-col gap-4 overflow-hidden bg-gradient-to-b ${product.gradient} relative transition-all duration-500 ease-in-out cursor-pointer
        ${isActive ? 'shadow-xl' : 'hover:shadow-2xl'} w-full lg:w-1/2 hover:scale-[1.02] hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.4)]`}
      style={{
        minHeight: '240px',
        height: 'auto',
        gap: '16px',
        ...
        width: isActive ? 'calc(60% - 12px)' : '40%'
      }}
      onClick={handleClick}
      role="button"
      tabIndex={0}
...
const MobileProductCard = ({
  ...
  return (
    <motion.div
      ...
      tabIndex={0}
      onKeyDown={handleKeyDown}
      initial={{ height: 'auto', minHeight: '140px', width: '100%' }}
      animate={{
        height: 'auto',
        minHeight: isActive ? '240px' : '140px',
        width: isActive ? '110%' : '100%',
        marginLeft: isActive ? '-5%' : '0',
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        viewport={{ once: true }}
        className="text-[28px] sm:text-[32px] md:text-[36px] lg:text-[40px] font-poppins font-semibold text-white leading-snug tracking-[-1px] lg:tracking-[-2px] text-left lg:text-left"
      >
        OUR PRODUCTS
      </motion.div>
      ...
      <motion.p 
        ...
        viewport={{ once: true }}
        className="font-poppins font-normal text-white/80 text-sm sm:text-base leading-[24px] max-w-full lg:max-w-[40%] text-left pb-12"
      >
      ...
      <div className="w-full flex lg:flex-col flex-col-reverse slg:flex-col items-start text-left gap-8 lg:gap-10 relative">
        ...
        <div className="hidden sm:flex flex-col lg:flex-col items-start text-left gap-8 lg:gap-10 w-full">
          <div className="flex flex-col lg:flex-row items-stretch justify-center w-full gap-4 lg:gap-6 transition-all duration-500 ease-in-out">
            {PRODUCTS_DATA.map(product => (
              <DesktopProductCard
                key={product.id}
                product={product}
          ...
          <div className="flex flex-col gap-4 sm:gap-6 md:gap-8 w-full" id={activeProduct === 'product1' ? 'intelligence-growth-features' : 'ai-agents-features'}>
            <div className="flex flex-col-reverse md:flex-row gap-6 p-4 sm:p-6 md:p-8 shadow-lg relative backdrop-blur-[30px] overflow-hidden"
              ...
              <div className="flex flex-col flex-1 min-h-[400px] lg:h-[600px] overflow-y-auto pt-[40px] lg:pt-[60px] scrollbar-hide">
                ...
                <Link href={START_SCALING_LINKS[activeProduct]}
